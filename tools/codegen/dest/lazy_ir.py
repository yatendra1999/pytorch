...
class {schema.node_name} : public {self.node_base} {{
 public:
  {schema.node_name}({node_ctor_args}, std::vector<lazy_tensors::Shape>&& shapes)
...
class GenLazyNativeFuncDefinition:
    ...
    def __call__(self, func: NativeFunction) -> List[str]:
        ...
        if func.structured or func.structured_delegate is not None:
            meta_out = """std::vector<lazy_tensors::Shape> shapes{lazy_tensors::Shape(out_meta.scalar_type(), out_meta.sizes().vec())};"""
            if returns_length > 1:
                this_shape = lambda i: f"lazy_tensors::Shape(std::get<{i}>(out_meta).scalar_type(), std::get<{i}>(out_meta).sizes().vec())"
                shapes_str = ','.join([this_shape(i) for i in range(returns_length)])
                meta_out = "std::vector<lazy_tensors::Shape> shapes{" + shapes_str + "};"
                # meta_out = f"""std::vector<lazy_tensors::Shape> shapes{lazy_tensors::Shape(std::get<i>(out_meta).scalar_type(), std::get<i>(out_meta.sizes().vec())};"""
        ...
        bridge_str = f"""auto result = bridge::AtenFromLtcTensor(lazy_{first_tensor.name}.CreateFrom(node,
            shapes.front().scalar_type()));"""
        if returns_length > 1:
            bridge_str = f"""std::vector<LazyTensor> lazy_tensors;
        for (int i = 0; i < {returns_length}; i++) {{
            lazy_tensors.push_back(lazy_{first_tensor.name}.CreateFrom(torch::lazy::Value(node, i), shapes[i].scalar_type()));
        }}
...
class ComputeShapeSignature:
    ...
    def shape_decl(self) -> str:
        return f"std::vector<lazy_tensors::Shape> compute_shape_{self.__decl_suffix()}"
